        ...
        V: de::Visitor<'de>,
    {
        match self {
            ...
            ParserNumber::String(x) => {
                visitor.visit_map(NumberDeserializer { number: x.into() })
            }
        }
    }
...
impl<'de, R: Read<'de>> Deserializer<R> {
    ...
    /// EOF is encountered.
    pub fn parse_whitespace(&mut self) -> Result<Option<u8>> {
        loop {
            match tri!(self.peek()) {
                Some(b' ' | b'\n' | b'\t' | b'\r') => {
                    self.eat_char();
    ...
    #[cfg(feature = "float_roundtrip")]
    pub(crate) fn do_deserialize_f32<'any, V>(
        &mut self,
        visitor: V,
    ) -> Result<V::Value>
    where
                    ...
                    match tri!(self.peek_or_null()) {
                        c @ b'0'..=b'9' => {
                            ...
                            if overflow!(significand * 10 + digit, u64::MAX) {
                                return Ok(ParserNumber::F64(tri!(self
                                    .parse_long_integer(
                                        positive,
                                        significand
                                    ),)));
    ...
    fn parse_number(
        ...
        Ok(match tri!(self.peek_or_null()) {
            b'.' => ParserNumber::F64(tri!(self.parse_decimal(
                positive,
                significand,
                0
            ))),
            b'e' | b'E' => ParserNumber::F64(tri!(self.parse_exponent(
                positive,
                significand,
                0
            ))),
    ...
    fn parse_decimal(
        ...
        if exponent_after_decimal_point == 0 {
            match tri!(self.peek()) {
                ...
                None => {
                    return Err(
                        self.peek_error(ErrorCode::EofWhileParsingValue)
                    );
                }
            }
        }
        ...
        loop {
            match tri!(self.peek_or_null()) {
                ...
                b'.' => {
                    self.eat_char();
                    return self
                        .parse_long_decimal(positive, self.scratch.len());
                }
                b'e' | b'E' => {
                    return self
                        .parse_long_exponent(positive, self.scratch.len());
                }
                _ => {
                    return self.f64_long_from_parts(
                        positive,
                        self.scratch.len(),
                        0,
                    );
    ...
    fn parse_long_exponent(
        ...
        while let c @ b'0'..=b'9' = tri!(self.peek_or_null()) {
            ...
            if overflow!(exp * 10 + digit, i32::MAX) {
                let zero_significand =
                    self.scratch.iter().all(|&digit| digit == b'0');
                return self.parse_exponent_overflow(
                    positive,
                    zero_significand,
                    positive_exp,
    ...
    fn f64_long_from_parts(
        ...
        let f = if self.single_precision {
            lexical::parse_truncated_float::<f32>(integer, fraction, exponent)
                as f64
        } else {
        ...
        loop {
            ...
            if frame == b'{' {
                match tri!(self.parse_whitespace()) {
                    ...
                    Some(_) => {
                        return Err(
                            self.peek_error(ErrorCode::KeyMustBeAString)
                        );
                    }
                    None => {
                        return Err(
                            self.peek_error(ErrorCode::EofWhileParsingObject)
                        );
                    }
                }
                tri!(self.read.ignore_str());
...
static POW10: [f64; 309] = [
    ...
    1e009, //
    1e010, 1e011, 1e012, 1e013, 1e014, 1e015, 1e016, 1e017, 1e018,
    1e019, //
    1e020, 1e021, 1e022, 1e023, 1e024, 1e025, 1e026, 1e027, 1e028,
    1e029, //
    1e030, 1e031, 1e032, 1e033, 1e034, 1e035, 1e036, 1e037, 1e038,
    1e039, //
    1e040, 1e041, 1e042, 1e043, 1e044, 1e045, 1e046, 1e047, 1e048,
    1e049, //
    1e050, 1e051, 1e052, 1e053, 1e054, 1e055, 1e056, 1e057, 1e058,
    1e059, //
    1e060, 1e061, 1e062, 1e063, 1e064, 1e065, 1e066, 1e067, 1e068,
    1e069, //
    1e070, 1e071, 1e072, 1e073, 1e074, 1e075, 1e076, 1e077, 1e078,
    1e079, //
    1e080, 1e081, 1e082, 1e083, 1e084, 1e085, 1e086, 1e087, 1e088,
    1e089, //
    1e090, 1e091, 1e092, 1e093, 1e094, 1e095, 1e096, 1e097, 1e098,
    1e099, //
    1e100, 1e101, 1e102, 1e103, 1e104, 1e105, 1e106, 1e107, 1e108,
    1e109, //
    1e110, 1e111, 1e112, 1e113, 1e114, 1e115, 1e116, 1e117, 1e118,
    1e119, //
    1e120, 1e121, 1e122, 1e123, 1e124, 1e125, 1e126, 1e127, 1e128,
    1e129, //
    1e130, 1e131, 1e132, 1e133, 1e134, 1e135, 1e136, 1e137, 1e138,
    1e139, //
    1e140, 1e141, 1e142, 1e143, 1e144, 1e145, 1e146, 1e147, 1e148,
    1e149, //
    ...
    1e209, //
    1e210, 1e211, 1e212, 1e213, 1e214, 1e215, 1e216, 1e217, 1e218,
    1e219, //
    1e220, 1e221, 1e222, 1e223, 1e224, 1e225, 1e226, 1e227, 1e228,
    1e229, //
    1e230, 1e231, 1e232, 1e233, 1e234, 1e235, 1e236, 1e237, 1e238,
    1e239, //
    1e240, 1e241, 1e242, 1e243, 1e244, 1e245, 1e246, 1e247, 1e248,
    1e249, //
...
impl<'de, R: Read<'de>, T: Deserialize<'de>> Iterator
    for SeqDeserializer<'de, R, T>
{
    type Item = Result<T>;

    fn next(&mut self) -> Option<Self::Item> {
        fn next_element<'de, T, R: Read<'de>>(
            seq: &mut SeqDeserializer<'de, R, T>,
        ) -> Result<Option<T>>
        where
            T: Deserialize<'de>,
        {
            fn has_next_element<'de, R: Read<'de>, T>(
                seq: &mut SeqDeserializer<'de, R, T>,
            ) -> Result<bool> {
                ...
                if peek == b']' {
                    ...
                    match tri!(seq.de.parse_whitespace()) {
                        ...
                        None => Err(seq
                            .de
                            .peek_error(ErrorCode::EofWhileParsingValue)),
                    }
                } else {
            ...
            if tri!(has_next_element(seq)) {
                Ok(Some(tri!(T::deserialize(&mut seq.de))))
            } else {
                Ok(None)
            }
        }
        ...
        fn has_next_key<'de, 'a, R: Read<'de> + 'a>(
            ...
            if peek == b'}' {
                ...
                match tri!(map.de.parse_whitespace()) {
                    ...
                    None => {
                        Err(map.de.peek_error(ErrorCode::EofWhileParsingValue))
                    }
                }
            } else {
            ...
            V: de::Visitor<'de>,
        {
            ...
            match tri!(self.de.peek()) {
                ...
                _ => {
                    return Err(self
                        .de
                        .peek_error(ErrorCode::ExpectedDoubleQuote))
                }
            }
